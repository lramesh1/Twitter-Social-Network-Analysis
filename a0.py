...
print ('Read', len(candidates), ' candidates:\n', '\n'.join(candidates))
...
def get_friends(twitter, screen_name):
    ...
    friend =[]
    b = "friends/ids"
    request =robust_request(twitter,b,{"screen_name":screen_name})
...
def print_num_friends(users):
    ...
    """
    for i in users:
       print ('%s %s' %(i['screen_name'], len(i['friends'])))
    ...
    for i in users:
    	for j in users:
    		if i['screen_name'] != j['screen_name']:
    			tuple1 = (i['screen_name'], j['screen_name'] , len(set(i['friends']).intersection(j['friends'])))
    			common_friends.append(tuple1)
    return sorted(common_friends, key = lambda x: (x[2]), reverse = True)
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for i in users:
         for j in users:
              if i['screen_name']== 'HillaryClinton' and  j['screen_name']== 'realDonaldTrump':
                      mutual=list( set(i['friends']) & set(j['friends']))

    request=robust_request(twitter,'users/show', {'user_id':mutual})
    for i in request:
...
def create_graph(users, friend_counts):
    ...
    for i in users:
    	for j in friend_counts:
    		if(j in i['friends'] and friend_counts[j]> 1):
